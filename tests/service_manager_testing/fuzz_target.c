...
uint64_t sm_request_id_generate(void) { return 0; }

void sm_request_id_set(uint64_t id) { (void)id; }

void sm_request_id_clear(void) {}

const char *sm_request_id_str(void) { return "mock-req-id"; }
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  case SM_MSG_REGISTER: {
    ...
    sm_register_req_t *msg =
        (sm_register_req_t *)(fuzz_data + sizeof(sm_hdr_t));
  ...
  case SM_MSG_HEARTBEAT: {
    ...
    sm_heartbeat_req_t *msg =
        (sm_heartbeat_req_t *)(fuzz_data + sizeof(sm_hdr_t));
    if (msg->service_name[0] == '\0') {
      break;
    }
  ...
  case SM_MSG_UNREGISTER: {
    ...
    sm_unregister_req_t *msg =
        (sm_unregister_req_t *)(fuzz_data + sizeof(sm_hdr_t));
