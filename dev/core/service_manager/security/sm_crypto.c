...
/* SHA-256 round constants (first 32 bits of fractional parts of cbrt of
 * primes 2..311) */
...
int sm_hmac_sha256(const uint8_t *key, size_t key_len, const uint8_t *data,
                   size_t data_len, uint8_t out[SM_HMAC_SIZE]) {
  ...
  sha256_init(&ctx);
  for (i = 0; i < 64; i++)
    k_pad[i] ^= 0x36; /* ipad = 0x36 repeated */
...
int sm_hmac_verify(const uint8_t *key, size_t key_len, const uint8_t *data,
                   size_t data_len, const uint8_t expected[SM_HMAC_SIZE]) {
...
static int generate_key(uint8_t *out, size_t len) {
  ...
  if (fd < 0) {
    sm_log(SM_LOG_ERROR, "crypto: cannot open /dev/urandom: %s",
           strerror(errno));
    return -1;
  }
...
int sm_crypto_init(void) {
  int fd;
  ssize_t n;
  const sm_config_t *cfg = sm_config_get(); // Get the config

  // Create a dynamic array of paths, putting the configured path first
  const char *key_paths[4] = {NULL};
  int path_count = 0;

  if (cfg && cfg->verify_key_file[0] != '\0') {
    key_paths[path_count++] = cfg->verify_key_file;
  }

// Add the hardcoded fallbacks
#ifndef SM_KEY_FILE
#define SM_KEY_FILE "/run/servicemanager.key"
#endif

  key_paths[path_count++] = SM_KEY_FILE;
  key_paths[path_count++] = "/tmp/servicemanager.key";
  key_paths[path_count] = NULL;
  ...
  while (key_paths[path_idx] != NULL) {
    ...
    if (fd >= 0) {
      ...
      sm_log(SM_LOG_WARN, "crypto: key file truncated at %s, regenerating",
             key_file);
    }
    ...
    fd = open(key_file, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC,
              SM_KEY_FILE_MODE);
    if (fd < 0) {
      sm_log(SM_LOG_WARN, "crypto: cannot create key file %s, trying fallback",
             key_file);
      path_idx++;
      continue;
    }
    ...
    if (n != SM_HMAC_KEY_SIZE) {
      sm_log(SM_LOG_WARN,
             "crypto: failed to write key file %s, trying fallback", key_file);
      path_idx++;
      continue;
    }
