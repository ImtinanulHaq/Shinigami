...
sm_config_t sm_config_default(void)
{
    ...
    strncpy(cfg.log_file, "/var/log/servicemanager.log", sizeof(cfg.log_file) - 1);
    strncpy(cfg.socket_path, "/run/servicemanager.sock", sizeof(cfg.socket_path) - 1);
...
int sm_config_load(const char* filename)
{
    ...
    if (!filename) filename = "/etc/servicemanager.conf";
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        if (!strcmp(section, "limits")) {
            if (!strcmp(key, "max_services")) g_config.max_services = parse_int(val);
            else if (!strcmp(key, "max_restarts")) g_config.max_restarts = parse_int(val);
            else if (!strcmp(key, "heartbeat_timeout")) g_config.heartbeat_timeout = parse_int(val);
        }
        else if (!strcmp(section, "logging")) {
            if (!strcmp(key, "level")) g_config.log_level = parse_int(val);
            else if (!strcmp(key, "max_size_mb")) g_config.log_max_size_mb = parse_int(val);
            else if (!strcmp(key, "file")) strncpy(g_config.log_file, val, sizeof(g_config.log_file) - 1);
        }
            ...
            else if (!strcmp(key, "global_capacity")) g_config.rate_global_capacity = parse_int(val);
        }
        else if (!strcmp(section, "management")) {
            if (!strcmp(key, "enable")) g_config.enable_management = parse_int(val);
            else if (!strcmp(key, "port")) g_config.management_port = parse_int(val);
        }
        else if (!strcmp(section, "threadpool")) {
            if (!strcmp(key, "size")) g_config.thread_pool_size = parse_int(val);
            else if (!strcmp(key, "queue_size")) g_config.work_queue_size = parse_int(val);
        }
    }
...
int sm_config_set_int(const char* key, int value)
{
    ...
    if (!strcmp(key, "max_services")) g_config.max_services = value;
...
{
    if (!cfg) return -1;
    if (cfg->max_services < 1 || cfg->max_services > 1024) return -1;
    if (cfg->max_restarts < 0) return -1;
    if (cfg->heartbeat_timeout < 1) return -1;
    if (cfg->thread_pool_size < 1 || cfg->thread_pool_size > 64) return -1;
    if (cfg->work_queue_size < 4 || cfg->work_queue_size > 4096) return -1;
    return 0;
}
