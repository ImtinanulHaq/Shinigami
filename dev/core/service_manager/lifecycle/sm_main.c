...
static int threadpool_init(void) {
  ...
  sm_log(SM_LOG_INFO, "main: thread pool started (%d workers, queue=%d)",
...
static void threadpool_shutdown(void) {
  ...
  /* Fix: Only shut down if workers were actually allocated */
  if (workers == NULL || work_queue.fds == NULL) {
    return;
  }
...
int sm_run(void) {
  ...
  /* Initialize all 10 enterprise features (discovery, watchdog, monitoring,
   * etc.) */
  ...
  if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, signal_fd, &ev) < 0) {
    sm_log(SM_LOG_ERROR, "main: epoll_ctl ADD signal_fd failed: %s",
           strerror(errno));
    close(signal_fd);
    close(epoll_fd);
    cleanup();
        ...
        if (s == sizeof(sinfo)) {
          if (sinfo.ssi_signo == SIGTERM) {
            ...
            if (sm_config_load(config_file) < 0) {
              sm_log(SM_LOG_WARN, "main: config reload failed for %s",
                     config_file);
            } else {
 ...
 */
static int _sm_transact(int fd, const void *req_payload,
                        size_t req_payload_size, uint16_t msg_type,
  ...
  if (key) {
    ...
    sm_hmac_sha256(key, SM_HMAC_KEY_SIZE, mac_input,
                   SM_HDR_HMAC_OFFSET + req_payload_size, hdr.hmac);
  }
  ...
  if (req_payload_size > 0) {
    ...
    if (n != (ssize_t)req_payload_size)
      return -1;
  }

  /* Receive reply */
  n = recv(fd, reply_buf, reply_size, MSG_WAITALL);
  if (n != (ssize_t)reply_size)
    return -1;
...
int sm_register(const char *name, const char *socket_path,
  ...
  if (_sm_transact(fd, &req, sizeof(req), SM_MSG_REGISTER, &reply,
                   sizeof(reply)) < 0) {
    close(fd);
    return SM_ERR_INVALID;
  }
...
int sm_lookup(const char *name, char *socket_path_out, char *ring_name_out) {
  ...
  if (_sm_transact(fd, &req, sizeof(req), SM_MSG_LOOKUP, &reply,
                   sizeof(reply)) < 0) {
    close(fd);
    return SM_ERR_INVALID;
  }
  ...
  if (socket_path_out)
    strncpy(socket_path_out, reply.socket_path, SM_MAX_PATH - 1);
  if (ring_name_out)
    strncpy(ring_name_out, reply.ring_name, SM_MAX_PATH - 1);
...
int sm_heartbeat(const char *name) {
  ...
  if (_sm_transact(fd, &req, sizeof(req), SM_MSG_HEARTBEAT, &reply,
                   sizeof(reply)) < 0) {
    close(fd);
    return SM_ERR_INVALID;
  }
...
int sm_unregister(const char *name) {
  ...
  if (_sm_transact(fd, &req, sizeof(req), SM_MSG_UNREGISTER, &reply,
                   sizeof(reply)) < 0) {
    close(fd);
    return SM_ERR_INVALID;
  }
