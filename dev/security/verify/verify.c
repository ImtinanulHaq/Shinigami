...
int verify_init_from_file(verify_context_t *ctx, const char *key_file) {
  ...
  if (!f) {
    syslog(LOG_ERR, "[verify] Failed to open key file '%s': %s", key_file,
           strerror(errno));
    return -1;
  }

  uint8_t key[MAX_KEY_SIZE];
  // Force it to read exactly 32 bytes to match the Service Manager's
  // SM_HMAC_KEY_SIZE
  size_t key_len = fread(key, 1, 32, f);
  fclose(f);
...
int verify_generate_key(const char *key_file) {
  ...
  if (!f) {
    syslog(LOG_ERR, "[verify] Failed to create key file '%s': %s", key_file,
           strerror(errno));
    secure_zero(key, sizeof(key));
    return -1;
  }
...
int verify_sign_message(verify_context_t *ctx, const void *data,
  ...
  if (!verify_check_rate_limit(ctx))
    return -1;
  ...
  int result = hmac_sign(ctx->master_key, ctx->master_key_len, sign_data,
                         total_len, auth->hmac);
...
int verify_check_message(verify_context_t *ctx, const void *data,
  ...
  if (ctx->enable_timestamp_check || ctx->strict_ordering) {
    int replay_result =
        replay_check(&ctx->replay_ctx, auth->timestamp, auth->sequence);
    if (replay_result != 1) {
  ...
  int result = hmac_verify(ctx->master_key, ctx->master_key_len, sign_data,
                           total_len, auth->hmac);
...
uint64_t verify_get_timestamp(void) { return replay_get_monotonic_time(); }
...
int verify_rotate_key(verify_context_t *ctx, const uint8_t *new_key,
  ...
  syslog(LOG_INFO, "[verify] Key rotated to generation %u",
         ctx->key_generation);
  return 0;
}
