...
static void sig_safe_write_int(char* buf, int* pos, int bufsize, int val)
{
    ...
    for (int i = tlen - 1; i >= 0; i--) {
        if (*pos < bufsize) buf[(*pos)++] = tmp[i];
    }
}

static void sig_safe_write_str(char* buf, int* pos, int bufsize, const char* s)
{
    while (*s && *pos < bufsize) {
        buf[(*pos)++] = *s++;
    }
...
static void handle_sigsys(int sig, siginfo_t* info, void* context)
{
    ...
    if (info->si_code == 1 && monitoring_enabled && log_fd >= 0) {
        char sig_buf[128];
        int pos = 0;
        sig_safe_write_str(sig_buf, &pos, (int)sizeof(sig_buf), "SECCOMP_VIOLATION: syscall=");
...
int seccomp_core_validate_architecture(scmp_filter_ctx ctx)
{
    ...
    if (seccomp_arch_add(ctx, arch) < 0) return -1;
