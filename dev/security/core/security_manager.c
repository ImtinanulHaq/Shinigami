...
int security_manager_apply_sandbox(security_manager_t *mgr,
                                   const sandbox_config_t *cfg) {
  if (!mgr || !cfg)
    return -1;

  if (mgr->seccomp_applied)
    return -1;
...
int security_manager_apply_capabilities(security_manager_t *mgr,
                                        const capabilities_config_t *cfg) {
  if (!mgr || !cfg)
    return -1;
...
int security_manager_apply_seccomp(security_manager_t *mgr,
                                   const seccomp_config_t *cfg) {
  if (!mgr || !cfg)
    return -1;
...
int security_manager_init_verify(security_manager_t *mgr,
                                 const char *key_file) {
  if (!mgr || !key_file)
    return -1;

  if (verify_init_from_file(&mgr->verify_ctx, key_file) < 0)
    return -1;
...
int security_manager_apply_all(security_manager_t *mgr,
                               const security_full_config_t *config) {
  if (!mgr || !config)
    return -1;

  /* 1. VERIFY LAYER: Read keys into memory while we still have full root access
   */
  ...
  /* 2. SANDBOX LAYER: Apply namespaces/cgroups (might restrict filesystem) */
  ...
  /* 3. CAPABILITIES LAYER: Drop root privileges to restricted UIDs */
  ...
  /* 4. SECCOMP LAYER: Apply system call filters (must absolutely be last) */
...
rollback:
  ...
  if (mgr->sandbox_applied && config->sandbox.enable_cgroups &&
      config->sandbox.cgroup_name) {
...
security_state_t security_manager_get_state(const security_manager_t *mgr) {
  if (!mgr)
    return SEC_STATE_UNINITIALIZED;
  return mgr->state;
}

security_full_config_t
security_manager_default_config(const char *service_name) {
  ...
  if (strcmp(service_name, "audio") == 0)
    ...
    syslog(LOG_WARNING,
           "[sec_mgr] unknown service '%s', using minimal seccomp profile",
           service_name);
...
int security_manager_load_config(security_full_config_t *config,
  ...
  if (!f) {
    syslog(LOG_ERR, "[sec_mgr] cannot open config '%s': %s", config_path,
           strerror(errno));
    return -1;
  }
