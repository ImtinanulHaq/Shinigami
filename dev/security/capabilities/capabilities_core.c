...
{
    switch (cap) {
        case MCAP_SYS_RAWIO: return "SYS_RAWIO";
        case MCAP_NET_BIND:  return "NET_BIND_SERVICE";
        case MCAP_SYS_ADMIN: return "SYS_ADMIN";
        case MCAP_SETUID:    return "SETUID";
        case MCAP_KILL:      return "KILL";
        case MCAP_NET_RAW:   return "NET_RAW";
        case MCAP_NONE:      return "NONE";
...
{
    __u32 bits = 0;
    if (flags & MCAP_SYS_RAWIO) bits |= (1U << CAP_SYS_RAWIO_NUM);
    if (flags & MCAP_NET_BIND)  bits |= (1U << CAP_NET_BIND_NUM);
    if (flags & MCAP_SYS_ADMIN) bits |= (1U << CAP_SYS_ADMIN_NUM);
    if (flags & MCAP_SETUID)    bits |= (1U << CAP_SETUID_NUM);
    if (flags & MCAP_KILL)      bits |= (1U << CAP_KILL_NUM);
    if (flags & MCAP_NET_RAW)   bits |= (1U << CAP_NET_RAW_NUM);
    return bits;
}
        ...
        if (!(word & cap_bit)) {
            if (prctl(PR_CAPBSET_DROP, cap, 0, 0, 0) < 0) {
                if (errno != EINVAL && errno != EPERM) {
                    syslog(LOG_WARNING, "[cap_core] bounding set drop cap %d: %s",
                           cap, strerror(errno));
                }
            }
        }
...
int capabilities_core_apply_config(const capabilities_config_t* config)
{
    ...
    capabilities_audit_log("START", MCAP_NONE, "beginning capability configuration");
    ...
    if (prctl(PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) < 0) {
        syslog(LOG_ERR, "[cap_core] PR_SET_NO_NEW_PRIVS failed: %s", strerror(errno));
        return -1;
    }
    ...
    if (config->target_uid > 0 && config->target_gid > 0) {
        ...
        if (current_uid == 0) {
            ...
            if (setgid(config->target_gid) < 0) {
                syslog(LOG_ERR, "[cap_core] setgid(%u) failed: %s",
                       config->target_gid, strerror(errno));
                return -1;
            }

            if (setuid(config->target_uid) < 0) {
                syslog(LOG_ERR, "[cap_core] setuid(%u) failed: %s",
                       config->target_uid, strerror(errno));
                return -1;
            }

            if (setuid(0) == 0) {
                syslog(LOG_CRIT, "[cap_core] SECURITY FAILURE: regained root privileges!");
                return -1;
            }
            ...
            syslog(LOG_WARNING, "[cap_core] Cannot change from uid %u to %u (not root)",
    ...
    syslog(LOG_INFO, "[cap_core] Configuration applied: effective=%s(0x%x) permitted=%s(0x%x) bounding=%s(0x%x) uid=%u gid=%u",
           capabilities_core_name(config->effective), config->effective,
           capabilities_core_name(config->permitted), config->permitted,
           capabilities_core_name(config->bounding), config->bounding,
           getuid(), getgid());

    capabilities_audit_log("COMPLETE", config->effective, "configuration successful");
    return 0;
}
...
{
    syslog(LOG_WARNING, "[cap_core] Using deprecated capabilities_core_drop_except - migrate to capabilities_core_apply_config");
    ...
    syslog(LOG_INFO, "[cap_core] %s", keep_flags ? "kept specific capabilities" : "all capabilities dropped");
...
int capabilities_core_set_ambient(cap_flags_t caps)
{
    ...
    static const struct { cap_flags_t flag; int cap_num; } cap_map[] = {
        {MCAP_SYS_RAWIO, CAP_SYS_RAWIO_NUM},
    ...
    for (int i = 0; cap_map[i].flag; i++) {
        if (caps & cap_map[i].flag) {
            if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, cap_map[i].cap_num, 0, 0) < 0) {
