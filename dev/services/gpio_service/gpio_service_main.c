/**
 * @file gpio_service_main.c
 * @brief GPIO service daemon entry point.
 *
 * Manages one daemon per GPIO chip; multiple pins are handled within a single
...
#include "gpio_service_security.h"
#include "gpio_service_loop.h"

static gpio_service_ctx_t  g_ctx;
...
static void do_shutdown(gpio_service_ctx_t *ctx, svc_ipc_t *ipc,
...
int main(int argc, char *argv[])
{
    ...
    if (!config_path) {
        fprintf(stderr, "gpio_service: -c <config_file> is required\n");
        usage(argv[0]);
        return EXIT_FAILURE;
    }

    /* ── Step 2: Config loading ───────────────────────────────────────── */
    if (service_config_load(&g_cfg, config_path, "GPIO_SERVICE") != 0) {
        fprintf(stderr, "gpio_service: failed to load config '%s'\n",
                config_path);
        return EXIT_FAILURE;
    }
    if (service_config_validate(&g_cfg, REQUIRED_KEYS) != 0) {
        fprintf(stderr, "gpio_service: config validation failed\n");
        return EXIT_FAILURE;
    }

    service_base_init(&g_ctx.base, "gpio_service");
    ...
    strncpy(g_ctx.base.log_path,
            service_config_get_string(&g_cfg, "server", "log_file",
                                      "/var/log/gpio_service.log"),
            sizeof(g_ctx.base.log_path) - 1);

    strncpy(g_ctx.chip,
            service_config_get_string(&g_cfg, "hardware", "chip",
                                      "gpiochip0"),
            sizeof(g_ctx.chip) - 1);
    g_ctx.pin_number   = service_config_get_uint32(&g_cfg, "hardware",
                              "pin_number",   0);
    g_ctx.direction    = service_config_get_uint32(&g_cfg, "hardware",
                              "direction",    1); /* default: output */
    g_ctx.initial_value = service_config_get_uint32(&g_cfg, "hardware",
                              "initial_value", 0);

    const char *edge_str = service_config_get_string(&g_cfg, "hardware",
                               "edge", "none");
    if      (strcasecmp(edge_str, "rising")  == 0) g_ctx.edge = 1;
    else if (strcasecmp(edge_str, "falling") == 0) g_ctx.edge = 2;
    else if (strcasecmp(edge_str, "both")    == 0) g_ctx.edge = 3;
    else                                            g_ctx.edge = 0;

    g_ctx.interrupt_timeout_ms = service_config_get_uint32(&g_cfg, "hardware",
                                      "interrupt_timeout_ms", 5000);

    /* ── Step 3: Daemonize ────────────────────────────────────────────── */
    if (!foreground) {
        if (service_base_daemonize(&g_ctx.base) != SVC_OK) {
            fprintf(stderr, "gpio_service: daemonize failed\n");
            return EXIT_FAILURE;
        }
    }

    /* ── Step 4: PID file ─────────────────────────────────────────────── */
    service_base_open_log("gpio_service", g_ctx.base.log_path);

    int rc = service_base_write_pid("gpio_service");
    ...
    LOG_INFO("gpio_service v%s starting (pid=%d, chip=%s, pin=%u)",
             SERVICE_LAYER_VERSION_STR, (int)g_ctx.base.pid,
    ...
    service_ipc_init(&g_ipc, "gpio_service", vkey);
    ...
    } else if (gpio_service_hal_init(&g_ctx) != SVC_OK) {
        LOG_ERR("HAL init failed");
        do_shutdown(&g_ctx, &g_ipc, "gpio_service", 0);
        return EXIT_FAILURE;
    }
    if (!skip_hal && gpio_service_hal_start(&g_ctx) != SVC_OK) {
        LOG_ERR("HAL start failed");
        do_shutdown(&g_ctx, &g_ipc, "gpio_service", 0);
        return EXIT_FAILURE;
    }
    if (!skip_hal) LOG_INFO("GPIO HAL started (chip=%s, pin=%u, dir=%s, edge=%s)",
             g_ctx.chip, g_ctx.pin_number,
    ...
    /* ── Step 10: Event loop ─────────────────────────────────────────── */
    rc = gpio_service_loop_run(&g_ctx, &g_ipc, &g_cfg);
    if (rc != SVC_OK)
        LOG_ERR("event loop exited with error: %s", svc_error_string(rc));

    /* ── Step 11: Graceful shutdown ──────────────────────────────────── */
    do_shutdown(&g_ctx, &g_ipc, "gpio_service", 1);
