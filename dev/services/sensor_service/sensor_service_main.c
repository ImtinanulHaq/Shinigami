...
int main(int argc, char *argv[])
{
    ...
    if (sensor_service_apply_security(&g_ctx) != SVC_OK) {
    ...
    } else if (sensor_service_hal_init(&g_ctx) != SVC_OK) {
        LOG_ERR("HAL init failed");
        do_shutdown(&g_ctx, &g_ipc, "sensor_service", 0);
        return EXIT_FAILURE;
    }
    if (!skip_hal && sensor_service_hal_start(&g_ctx) != SVC_OK) {
        LOG_ERR("HAL start failed");
        do_shutdown(&g_ctx, &g_ipc, "sensor_service", 0);
        return EXIT_FAILURE;
    }
    if (!skip_hal) LOG_INFO("sensor HAL on %s started (type=%s, rate=%u Hz)",
