...
static void *collector_thread(void *arg)
{
    ...
    while (!c->stop_flag) {
        ...
        case COLLECTOR_STATE_WAITING:
            if (collector_sleep_ms(c, c->retry_ms)) goto done;
            collector_set_state(c, COLLECTOR_STATE_CONNECTING);
            break;
        ...
        case COLLECTOR_STATE_STALE: {
            ...
            /* Check if stale */
            if (c->state == COLLECTOR_STATE_LIVE &&
                c->last_update_ms &&
...
{
    if (g_collector_count >= COLLECTOR_TABLE_MAX) return -1;
    if (!c->retry_ms) c->retry_ms = 2000;
                                 ...
                                 uint32_t *offline)
{
    ...
    for (uint32_t i = 0; i < g_collector_count && n < max; i++) {
        ...
        switch (c->state) {
        case COLLECTOR_STATE_LIVE:                         l++; break;
        case COLLECTOR_STATE_STALE:                        s++; break;
        case COLLECTOR_STATE_OFFLINE:
        case COLLECTOR_STATE_WAITING:
        case COLLECTOR_STATE_CONNECTING:
        case COLLECTOR_STATE_SYNCING:                      o++; break;
        }
    }
