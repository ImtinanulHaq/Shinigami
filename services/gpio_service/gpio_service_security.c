/**
 * @file gpio_service_security.c
 * @brief Security profile application for the GPIO service.
 *
...
int gpio_service_apply_security(gpio_service_ctx_t *ctx)
{
    ...
    SVC_INFO("applying security profile for 'gpio'");

    /*
     * No dedicated seccomp profile for GPIO in the current Security Module;
     * security_manager_default_config falls back to SERVICE_TYPE_MINIMAL.
     */
    ...
    SVC_INFO("GPIO security profile applied (state=%d)",
