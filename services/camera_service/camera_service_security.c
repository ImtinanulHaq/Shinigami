/**
 * @file camera_service_security.c
 * @brief Security profile application for the camera service.
 *
...
int camera_service_apply_security(camera_service_ctx_t *ctx)
{
    ...
    SVC_INFO("applying security profile for 'camera'");

    security_full_config_t sec_cfg = security_manager_default_config("camera");
    ...
    SVC_INFO("camera security profile applied (state=%d)",
